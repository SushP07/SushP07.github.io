...
### 🚀 Featured Architecture: MLflow on AWS
*A secure, scalable MLOps platform enabling collaborative experiment tracking.*

![MLflow Architecture](./pipeline_v2.svg)

**Core Capabilities:**
* **Centralized Tracking:** Decoupled experiment logs from local machines to a collaborative EC2 server.
* **Security-by-Design:** Implemented **IAM Roles** to manage S3 access without hardcoded credentials.
* **Artifact Management:** Automated offloading of models (`.pkl`) and plots to S3 storage.

| Component | AWS Service | Role |
| :--- | :--- | :--- |
| **Compute** | `EC2 (t2.micro)` | Hosts the MLflow tracking server on Port 5000. |
| **Storage** | `S3 Bucket` | Durable storage for model binaries and artifacts. |
| **Security** | `IAM Roles` | Zero-key access control for EC2-to-S3 communication. |
